...
suite('Copilot Terminal Detection Test Suite', () => {
	vscode.window.showInformationMessage('Running Copilot Terminal Detection tests');
	...
	test('Extension should activate without errors', () => {
		...
		if (extension && extension.isActive) {
			assert.ok(true, 'Extension is active');
		} else {
			assert.ok(true, 'Extension loaded'); // Extension may not be active in test environment
		}
	});
});
